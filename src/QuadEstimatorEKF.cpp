...
//  ekfState(6) = ekfState(6) + dtIMU * gyro.z;	// yaw
  Quaternion<float> attitudeQaut = Quaternion<float>::FromEuler123_RPY(rollEst, pitchEst, ekfState(6));

  attitudeQaut.IntegrateBodyRate(gyro, dtIMU);

  float predictedPitch = attitudeQaut.Pitch();
  float predictedRoll = attitudeQaut.Roll();

  ekfState(6) = attitudeQaut.Yaw();
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  V3F scaled0 = RbgPrime(0) * accel;
  V3F scaled1 = RbgPrime(1) * accel;
  V3F scaled2 = RbgPrime(2) * accel;
  ...
  gPrime(1,4) = dt;
  gPrime(2,5) = dt;
  gPrime(3,6) = (scaled0.x + scaled0.y + scaled0.z) * dt;
  gPrime(4,6) = (scaled1.x + scaled1.y + scaled1.z) * dt;
  gPrime(5,6) = (scaled2.x + scaled2.y + scaled2.z) * dt;
